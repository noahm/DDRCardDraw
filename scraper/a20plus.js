...
const ZIV_EXTREME = 'https://zenius-i-vanisher.com/v5.2/gamedb.php?gameid=5156&show_notecounts=1&sort=&sort_order=asc';
...
function scrapeSongData(dom) {
  const numbers = []
  dom.window.document.querySelectorAll('th[colspan="11"] span').forEach((node) => numbers.push(Number(node.textContent.match(/^[0-9]*/)[0])))
...
function createSongData(songLink, title){
  ...
  const a20Data = a20DataList.find((target) => target.name === songLink.text.trim())
  const songData = {
...
function writeDataFile(songs) {
  const fs = require('fs');
  fs.writeFileSync('./a20plus.json', JSON.stringify(songs, null, 2));
