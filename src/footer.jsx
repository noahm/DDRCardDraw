...
function Icon({ src, title }) {
  return (
    <figure title={title} alt={title} dangerouslySetInnerHTML={{ __html: src }} className={styles.icon} />
  );
}
...
export function Footer() {
  return (
    <footer className={classNames(styles.footer, globalStyles.padded)}>
      <div>Songs up to date as of 1/08/2019. <a href="#" onClick={showCredits}>Credits</a></div>
      <div>
        <a href="https://m.me/noah.manneschmidt" target="_blank"><Icon src={fbLogo} title="Contact me on Facebook Messenger" /></a>
        &nbsp;
        <a href="https://twitter.com/Cathadan" target="_blank"><Icon src={twitterLogo} title="Contact me on Twitter" /></a>
        &nbsp;
        <a href="https://github.com/noahm/DDRCardDraw"><Icon src={githubLogo} title="Contribute on Github" /></a>
      </div>
    </footer>
  );
}
